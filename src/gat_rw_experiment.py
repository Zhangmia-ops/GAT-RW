...
def run_pipeline(train_file, test_file, output_json, params):
    ...
    rw_graph = DynamicWeightedGraph() # Teammate's graph structure
    ...
    gat_predictions = [] # List of (u, v, w_pred)
    true_values = []     # List of true weights
    test_pairs = []
    ...
    with open(test_file, 'r') as f:
        for line in f:
            ...
            if u < num_nodes and v < num_nodes:
                ...
                w_pred = 0.0 # Handle isolated/new nodes
    ...
    # Only sample nodes involved in test edges
    rw_probs = {} # {(u,v): avg_prob}
    
    # Extract source nodes involved in test pairs for random walks
    unique_test_nodes = set()
    for u, v in test_pairs:
        unique_test_nodes.add(u)
        unique_test_nodes.add(v)
        
    node_pi_cache = {} # Cache stationary distribution per node
    ...
    # Compute bidirectional probability for each test edge
    for u, v in test_pairs:
    ...
    # 7. Calculate metrics and export JSON
    ...
    for i, (u, v, w_final) in enumerate(final_results_list):
        raw_results.append({
            "u": u,
            "v": v,
            "true_w": round(float(true_values[i]), 6),  # Keep only 6 decimal places to save space, more digits not necessary
